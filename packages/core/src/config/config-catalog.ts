...
export const CONFIG_FIELD_DOCS: readonly ConfigFieldDoc[] = [
  ...
  field("compaction.inputBudget", "enum", "summary uses calibrated hard input capacity; context uses the legacy fill target. Both reserve summary output.", false, ["summary", "context"]),
  ...
  field("compaction.transport.maxAttempts", "integer", "Total attempts for transport failures; default 3 (initial call plus up to two retries).", false),

...
export const CONFIG_FIELD_DOCS: readonly ConfigFieldDoc[] = [
  ...
  field("compaction.segmentTargetTokens", "integer", "Target size of each source segment before framing; the request cap still applies.", false),
  field("compaction.maxRequestInputTokens", "integer", "Hard estimated-input cap per compaction provider attempt; effective cap is also bounded by the active model. Default 64000.", false),
  field("compaction.maxOperationInputTokens", "integer", "Hard cumulative estimated-input cap across all provider attempts and retries in one compaction. Default 2000000.", false),
  field("compaction.preserveRecentVerbatim", "boolean", "Must remain true for non-destructive compaction.", false),
  field("compaction.directUpdate", "boolean", "Use one validated previous-summary plus source update when the whole prompt fits; default true.", false),
  field("compaction.inputBudget", "enum", "context (default) uses the ordinary fill target; summary opts into calibrated hard input capacity. Both reserve summary output.", false, ["summary", "context"]),
  field("compaction.maxConcurrentSegments", "integer", "Independent segment requests in flight, integer 1–2; default 2. Aggregates remain ordered.", false),
  ...
  field("compaction.transport.maxAttempts", "integer", "Total attempts for transport failures; default 4 (initial call plus up to three retries), matching Pi's standard retry count.", false),
  field("compaction.transport.baseDelayMs", "integer", "Base backoff ms, doubled per attempt; default 2000, capped at 60000.", false),

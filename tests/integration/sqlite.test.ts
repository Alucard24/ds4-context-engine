...
describe("ContextDatabase", () => {
  it("applies schema migrations transactionally and can reopen idempotently", () => {
    ...
    expect(first.migrations).toHaveLength(7);
    ...
    expect(first.health()).toMatchObject({ ok: true, schemaVersion: 7, foreignKeys: true });
    first.close();
    first.close();

    const second = ContextDatabase.open(path, { now: 1_724_544_100_000 });
    expect(second.migrations).toHaveLength(7);
